...
function listKeyboard() {
  ...
  for (const { r, s } of rows) {
    const mark = r.track === 'sryv' ? ' ·срыв' : '';
    kb.text(`${s.icon} ${r.name} — ${s.percent}%${mark}`, `r:${r.id}`).row();
  }
  return kb;
}
...
bot.command(['spisok', 'list', 'residents'], async (ctx) => {
  ...
  await ctx.reply('🔴 отстаёт · 🟡 в норме · 🟢 спешит\n\nКого отмечаем?', { reply_markup: kb });
});

bot.callbackQuery('list', async (ctx) => {
  await ctx.answerCallbackQuery();
  await ctx.editMessageText('🔴 отстаёт · 🟡 в норме · 🟢 спешит\n\nКого отмечаем?', {
...
async function showResident(ctx, r) {
  ...
  kb.text('Карточка', `c:${r.id}`).text('« Список', 'list');
...
function statusLine(s) {
  if (s.state === 'behind') return `Отстаёт на ${-s.diff} ед. от графика`;
  if (s.state === 'ahead') return `Спешит: на ${s.diff} ед. впереди графика`;
  return 'Идёт по графику';
}
...
async function showGroup(ctx, r, ti, gi) {
  ...
  kb.row();
  if (gp.done < gp.total) kb.text('Отметить все', `all:${r.id}:${ti}:${gi}`);
  kb.text('« К заданию', `t:${r.id}:${ti}`).row();
...
bot.command(['svodka', 'today'], async (ctx) => {
  ...
  const behind = rows.filter((x) => x.s.state === 'behind').sort((a, b) => a.s.diff - b.s.diff);
  const ahead = rows.filter((x) => x.s.state === 'ahead').sort((a, b) => b.s.diff - a.s.diff);
  ...
  t += `Резидентов: ${rows.length}\n\n`;

  t += `<b>🔴 Отстают — ${behind.length}</b>\n`;
  t += behind.length
    ? behind.map((x) =>
        `${x.r.name} — ${x.s.percent}%, минус ${-x.s.diff} ед.` +
        `\n   ${P.currentTask(x.r)?.short || 'нет задания'}, до выписки ${x.s.daysLeft} дн.`
      ).join('\n') + '\n'
    : 'нет\n';

  t += `\n<b>🟢 Спешат — ${ahead.length}</b>\n`;
  t += ahead.length
    ? ahead.map((x) => `${x.r.name} — ${x.s.percent}%, плюс ${x.s.diff} ед.`).join('\n') + '\n'
    : 'нет\n';

  t += `\n<b>🟡 По графику — ${ok.length}</b>\n`;
