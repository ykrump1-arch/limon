...
/data — поправить даты
/kadr — персонал: добавить и удалить
...
bot.command(['kadr', 'staff'], async (ctx) => {
  if (!isAdmin(ctx.from.id)) return ctx.reply('Только руководитель.');
  const arg = (ctx.match || '').trim();

  if (arg) {
    const added = [];
    for (const part of arg.split(',').map((x) => x.trim()).filter(Boolean)) {
      const [name, id] = part.split(':').map((x) => (x || '').trim());
      if (!name) continue;
      const { staff: st, created } = db.addStaff(name, /^\d+$/.test(id || '') ? id : null);
      added.push(`${created ? 'добавлен' : 'обновлён'}: ${st.name}${st.tgId ? ' — ' + st.tgId : ''}`);
    }
    db.flush();
    return ctx.reply(added.join('\n') || 'Формат: /kadr Имя или /kadr Имя:123456789');
  }
  await showKadr(ctx);
});

function kadrKeyboard() {
  const kb = new InlineKeyboard();
  for (const st of db.staff()) {
    kb.text(`${st.tgId ? '🔗' : '⬜'} ${st.name}`, `kd:${st.id}`).row();
  }
  return kb;
}

async function showKadr(ctx, edit = false) {
  const t =
    '<b>Персонал</b>\n\n' +
    db.staff().map((st) => `${st.tgId ? '🔗' : '⬜'} ${st.name}${st.tgId ? ` — <code>${st.tgId}</code>` : ''}`).join('\n') +
    '\n\nНажми на имя, чтобы удалить.\n\n' +
    'Добавить:\n<code>/kadr Ахмад</code>\n<code>/kadr Ахмад:123456789</code>\n' +
    '<code>/kadr Ахмад, Модератор</code>';
  const opts = { reply_markup: kadrKeyboard(), parse_mode: 'HTML' };
  if (edit) { try { return await ctx.editMessageText(t, opts); } catch {} }
  await ctx.reply(t, opts);
}

bot.callbackQuery(/^kd:(\w+)$/, async (ctx) => {
  if (!isAdmin(ctx.from.id)) return ctx.answerCallbackQuery({ text: 'Только руководитель' });
  const st = db.staffById(ctx.match[1]);
  if (!st) return ctx.answerCallbackQuery({ text: 'Не найден' });
  await ctx.answerCallbackQuery();
  await ctx.editMessageText(`Удалить «${st.name}» из персонала?\n\nОтметки, которые он уже сделал, останутся.`, {
    reply_markup: new InlineKeyboard().text('Удалить', `kdx:${st.id}`).text('Отмена', 'kdno'),
  });
});

bot.callbackQuery(/^kdx:(\w+)$/, async (ctx) => {
  if (!isAdmin(ctx.from.id)) return ctx.answerCallbackQuery({ text: 'Только руководитель' });
  const gone = db.removeStaff(ctx.match[1]);
  db.flush();
  await ctx.answerCallbackQuery({ text: gone ? `Удалён: ${gone.name}` : 'Не найден' });
  await showKadr(ctx, true);
});

bot.callbackQuery('kdno', async (ctx) => {
  await ctx.answerCallbackQuery();
  await showKadr(ctx, true);
});
