...
// Главная функция сводки: отстаёт / спешит / в норме
export function status(r) {
  ...
  const diff = o.done - expectedUnits;                 // + спешит, − отстаёт
  const diffPct = Math.round((diff / o.total) * 100);

  let state = 'ok';
  if (diffPct <= -7) state = 'behind';
  else if (diffPct >= 7) state = 'ahead';

  // сколько единиц надо закрывать в день, чтобы успеть
