...
const isStaff = (id) => DEMO || isAdmin(id) || STAFF_IDS.includes(String(id));
...
const MENU = new Keyboard()
  ...
  .text('📊 Сводка').text('ℹ️ Помощь')
  .resized().persistent();

// Кто сейчас вводит текст технички
...
bot.hears('🔧 Техничка', (ctx) => showTech(ctx));
...
bot.callbackQuery('tadd', async (ctx) => {
  waiting.add(String(ctx.from.id));
  await ctx.answerCallbackQuery();
  await ctx.reply(
...
bot.hears('📋 Задания', (ctx) => showList(ctx));
...
bot.on('message:text', async (ctx, next) => {
  ...
  if (!waiting.has(uid)) return next();
  if (ctx.message.text.startsWith('/')) { waiting.delete(uid); return next(); }

  waiting.delete(uid);
  const lines = ctx.message.text.split('\n').map((x) => x.trim()).filter(Boolean);
  for (const line of lines) {
    db.addTech({ text: line, by: ctx.from.id, byName: ctx.from.first_name });
  }
  db.flush();
