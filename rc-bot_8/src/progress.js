...
// Установить отметку без переключения — нужно для импорта отчётов
export function setUnit(resident, code, gi, ui, value = true) {
  const task = byCode[code];
  const rec = (resident.tasks[code] ||= { issuedAt: today(), closedAt: null, progress: {} });
  const arr = (rec.progress[gi] ||= new Array(task.groups[gi].count).fill(false));
  const was = arr[ui];
...
// Если человек сдаёт задание N, значит всё до него уже закрыто.
// Возвращает, сколько заданий пришлось дозакрыть.
export function closePreceding(resident, code, when = null) {
  ...
  for (let i = 0; i < idx; i++) {
    const t = list[i];
    const tp = taskProgress(resident, t.code);
    if (tp.complete) continue;
    resident.tasks[t.code] = {
      issuedAt: resident.tasks[t.code]?.issuedAt || resident.startedAt,
      closedAt: when || today(),
